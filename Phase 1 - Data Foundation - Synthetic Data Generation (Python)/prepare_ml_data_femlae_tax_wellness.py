...
def transform_data(profiles_df, upi_df, weights):
    ...
    # --- MODIFIED: Income Consistency Calculation ---
    print("Calculating fair income consistency (now penalizing income gaps)...")
    
    salary_credits = upi_df[upi_df['description'].isin(MERCHANT_CATEGORIES['income'])]
    
    # Still exclude maternity leave months to be fair
    working_months_income = salary_credits[salary_credits['on_maternity_leave'] == 0]
    
    # Pivot to get monthly income. This will have NaNs for non-paid months.
    monthly_income = working_months_income.groupby(['user_id', 'month'])['amount'].sum().unstack()
    
    # --- THE CRUCIAL CHANGE ---
    # Fill non-paid months with 0. This forces the std() to account for income gaps.
    monthly_income = monthly_income.fillna(0)
    
    income_std_dev = monthly_income.std(axis=1)
    income_mean = monthly_income.mean(axis=1)
    
    # Coefficient of Variation: A zero-income month will now correctly drive this value up.
    # We add a small epsilon to the mean to prevent division by zero for users with no income at all.
    income_consistency_cv = (income_std_dev / (income_mean + 1e-6)).fillna(0)
    
    # Apply the lenient consistency score logic
    income_consistency_score = 1 / (1 + income_consistency_cv)
    # The forgiving threshold for users with very low variance (but who are paid consistently)
    ...
    features_to_merge = [
        total_income, total_debits, income_consistency_score, 
        food_spending, ott_spending, loan_repayments, bnpl_repayments
    ]
    ...
    ml_df['debt_burden_ratio'] = (ml_df['total_debits'] / (ml_df['total_income'] + 1e-6)).replace([np.inf, -np.inf], 0).fillna(0)
